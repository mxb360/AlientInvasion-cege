...
void show_game_info(GameStats *stats)
{
    ...
    cege_xyprintf(10, 10, "生命：%d", stats->plane_life);
    cege_xyprintf(10, 50, "得分： %d", stats->score);
    cege_xyprintf(10, 90, "击落： %d", stats->plane_shoot);
    cege_xyprintf(10, 130, "错失： %d", stats->plane_lost);
    ...
    if (stats->is_game_pause || !stats->game_active || stats->game_over) {
        ...
        if (stats->is_game_pause)
            cege_rectprintf(WINDOW_WIDTH/2 - 200, WINDOW_HEIGHT/2, 400, 100, "继续游戏");
        else {
            cege_rectprintf(WINDOW_WIDTH/2 - 200, WINDOW_HEIGHT/2, 400, 100, "开始游戏");
            setbkmode(TRANSPARENT);
            if (stats->game_over)
                cege_rectprintf(WINDOW_WIDTH/2-225, WINDOW_HEIGHT/2-150, 450, 100, "GAME OVER");
            else
                cege_rectprintf(WINDOW_WIDTH/2-250, WINDOW_HEIGHT/2-150, 500, 100, "外星人入侵");
        }
    }
}
