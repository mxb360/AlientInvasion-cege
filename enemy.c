...
void enemy_init(Enemies *enemy)
{
     ...
     for (i = 0; i < ENEMY_TYPES; i++) {
         ...
         _images[i] = cege_newimage(_enemy_width[i], _enemy_height[i]);

         getimage(image, images_str[i]);
         cege_putimage(_images[i], 0, 0, _enemy_width[i], _enemy_height[i],
                   image, 0, 0, enemy_src_width[i], enemy_src_height[i], SRCCOPY);
         delimage(image);
     }
...
void enemy_draw(void)
{
    ...
    for (i = 0; i < ENEMY_ALLOWED; i++)
        if (_enemy->enemies[i])
            cege_putimage_transparent(NULL, _images[_enemy->enemies[i]->type], 
                                       _enemy->enemies[i]->rect.left,
                                       _enemy->enemies[i]->rect.top, BLACK, 0, 0, 0, 0);
}
