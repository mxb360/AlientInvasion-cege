...
void plane_init(Plane *plane)
{
    ...
    _plane = plane;
    _plane->image = cege_newimage(PLANE_WIDTH, PLANE_HEIGHT);

    getimage(image, PLANE_IMAGE);
    cege_putimage(_plane->image, 0, 0, PLANE_WIDTH, PLANE_HEIGHT, 
             image, 0, 0, PLANE_SRC_WIDTH, PLANE_SRC_HEIGHT, SRCCOPY);
...
void plane_draw(GameStats *stats)
{
    if (!stats->game_active || stats->is_plane_hit)
        return;
    cege_putimage_transparent(NULL, _plane->image, _plane->rect.left, _plane->rect.top, BLACK,0,0,0,0);
}
