...
{ 
    ...
    _bullets = bullets;
    _bullets->image = cege_newimage(BULLET_WIDTH, BULLET_HEIGHT);

    getimage(image, BULLET_IMAGE);
    cege_putimage(_bullets->image, 0, 0, BULLET_WIDTH, BULLET_HEIGHT,
        image, 0, 0, BULLET_SRC_WIDTH, BULLET_SRC_HEIGHT, SRCCOPY);
...
void bullets_draw(void)
{
    ...
    for (i = 0; i < BULLET_ALLOWED; i++)
        if (_bullets->bullets[i])
            cege_putimage_transparent(NULL, _bullets->image, _bullets->bullets[i]->left, 
                                       _bullets->bullets[i]->top, BLACK, 0, 0, 0, 0);
}
