                      ...
                      int w, int h, int cols, int lines, int cnt, int updatetime)
{
    ...
    dynamic->image = cege_newimage(w * cols, h * lines);
    cege_putimage(dynamic->image, 0, 0, w * cols, h * lines, image, 0, 0, src_w, src_h, SRCCOPY);
    delimage(image);
...
void dynamic_draw(Dynamic *dynamic)
{
    ...
    cege_putimage_transparent(NULL, dynamic->image, dynamic->rect.left, dynamic->rect.top,
